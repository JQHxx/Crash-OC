...
- (void)ddy_PresentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (@available(iOS 13.0, *)) {
        if (viewControllerToPresent.ddy_AutoSetModalPresentationStyle) {
            NSLog(@"枚举值 %ld", (long)viewControllerToPresent.modalPresentationStyle);
            // iOS 13  默认样式UIModalPresentationAutomatic = -2  之前 版本UIModalPresentationFullScreen = 0
            // iOS 13 多了一个新的枚举类型 UIModalPresentationAutomatic，并且是modalPresentationStyle的默认值，而UIModalPresentationAutomatic实际是表现是在 iOS 13的设备上被映射成UIModalPresentationPageSheet
